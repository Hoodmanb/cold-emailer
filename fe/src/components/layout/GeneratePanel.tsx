...
import {
  ...
  Select, MenuItem, FormControl, useMediaQuery, useTheme,
} from "@mui/material";
import {
...
export default function GeneratePanel({ jobId, onComplete, initialAts }: GeneratePanelProps) {
  ...
  useEffect(() => {
    if (initialAts && initialAts.score !== undefined && step === "idle") {
      setAts(initialAts);
      setStep("ats-done");
    }
  }, [initialAts, step]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (configError) {
        ...
        setStep("error");
        showSnackbar(`${configError} Open Settings â†’ AI Workflows to configure.`, "error");
        return;
      }
  ...
  // Render ATS Score + Missing Keywords (used in both ats-done and done states)
    ...
    <>
            ...
            >
              ...
              >
                <FormControlLabel
                  ...
                  sx={{ m: 0, flex: 1 }}
                />
                <FormControl size="small" sx={{ minWidth: 88, width: { xs: "100%", sm: "auto" } }} disabled={!selectedTypes.includes(type)}>
                  <Select
      ...
      {isTokenUser && selectedTypes.length > 0 && (
        <Alert severity={creditBalance < estimateSelectionCost(selectedTypes, tailoringLevel) ? "warning" : "info"} sx={{ borderRadius: 2 }}>
          ...
          {" Â· "}Balance after: <strong>{Math.max(0, creditBalance - estimateSelectionCost(selectedTypes, tailoringLevel))}</strong>
          {" Â· "}Current: <strong>{creditBalance}</strong>
        </Alert>
      )}
      ...
      <Button
        ...
        disabled={!selectedTypes.length || genLoading}
        fullWidth
  ...
  // Render generated documents list
  const renderGeneratedDocs = () => (
    <>
      ...
      <Stack gap={1}>
        {generatedDocs.map((doc) => (
          ...
          >
            ...
            <Stack direction="row" gap={0.5} width={{ xs: "100%", sm: "auto" }} justifyContent={{ xs: "flex-end", sm: "flex-start" }}>
              <Button size="small" variant="contained" startIcon={<Edit3 size={12} />} onClick={() => setEditingDoc(doc)} sx={{ borderRadius: 2, fontSize: "0.7rem", py: 0.25, flex: { xs: 1, sm: "auto" } }}>
                Edit
              </Button>
              <Button size="small" variant="outlined" disabled={downloadingId === doc.id} onClick={() => handleDownloadDoc(doc)} sx={{ borderRadius: 2, fontSize: "0.7rem", py: 0.25, flex: { xs: 1, sm: "auto" } }}>
      ...
      <Button size="small" startIcon={<RefreshCw size={14} />} onClick={goBackToSelection} sx={{ fontWeight: 700 }}>
        Generate More
      </Button>
    </>
  );
  ...
  return (
    <>
      <Paper variant="outlined" sx={{ p: { xs: 2, sm: 3 }, borderRadius: 4, borderColor: step === "done" ? "success.main" : "primary.main", borderWidth: 1.5 }}>
        <Stack gap={2.5}>
          <Stack direction="row" alignItems="center" gap={1}>
          ...
          {/* ATS Results - show in both ats-done and done states */}
          {(step === "ats-done" || step === "done") && ats && renderAtsResults()}

          {/* Document Selection - only in ats-done */}
          {step === "ats-done" && renderDocumentSelection()}

          {/* Generated Documents - only in done */}
          {step === "done" && renderGeneratedDocs()}
