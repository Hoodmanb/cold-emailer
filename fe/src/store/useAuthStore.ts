...
// /** Ensure persist hydration completes before auth bootstrap runs. */
...
const useAuthStore = create<AppState>()(
  persist(
    ...
    }),
    {
      name: "career-bot-auth",
      storage: createJSONStorage(() => localStorage),
      // FIX: Never persist isAuthenticated — it MUST be derived from token + user
      partialize: (state) => ({
        user: state.user,
        userProfile: state.userProfile,
        token: state.token,
      ...
      onRehydrateStorage: () => (state, error) => {
        if (error) {
          ...
          // FIX: Derive isAuthenticated from actual data, never trust persisted flag
          const hasValidAuth = !!state.token && !!state.user?.id;
          state.isAuthenticated = hasValidAuth;

          if (state.token) {
            console.log("[AuthStore] Rehydrated token from storage, auth valid:", hasValidAuth);
            syncAuthCookie(state.token);
          } else {
            console.log("[AuthStore] Rehydrated with no active session");
            // Clean up any stale user data if no token
