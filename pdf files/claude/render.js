...
class CVRenderer {
  ...
  renderHeader() {
    ...
    const contactData = [
      { label: "Phone:", value: contact.phone, type: "text" },
      { label: "Email:", value: contact.email, type: "email" },
      { label: "LinkedIn:", value: contact.linkedIn, type: "linkedin" },
    ];

    contactItems.forEach((item, index) => {
      if (contactData[index]) {
        item.querySelector(".contact-label").textContent =
          contactData[index].label;
        ...
        } else if (contactData[index].type === "linkedin") {
          if (contactData[index].value) {
            valueEl.textContent = "View Profile";
            valueEl.setAttribute("href", contactData[index].value);
          } else {
            // Hide the entire LinkedIn row if no URL provided
  ...
  renderLeadership() {
    ...
    container.innerHTML = this.data.leadership
      .map(
        (item) => `
        <div class="responsibility-item">
            ${this.escapeHtml(item)}
  ...
  renderWorkHistory() {
    ...
    container.innerHTML = this.data.workHistory
      .map(
        (job) => `
            <div class="job-entry">
  ...
  renderEducation() {
    ...
    container.innerHTML = this.data.education
      .map(
        (edu) => `
  ...
  renderMscModules() {
    ...
    container.innerHTML = this.data.mscModules
      .map(
        (item) => `
        <div class="responsibility-item">
            ${this.escapeHtml(item)}
  ...
  renderResearch() {
    ...
    if (this.data.research && this.data.research.length > 0) {
      container.innerHTML = this.data.research
        .map(
          (item) => `
